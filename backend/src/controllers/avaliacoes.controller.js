...
function validarBody(body) {
  ...
  if (!body.id_paciente) {
    erros.push('id_paciente é obrigatório.');
  }

  if (!Array.isArray(body.respostas)) {
    erros.push('respostas deve ser um array.');
  }
  ...
  if (Array.isArray(body.respostas)) {
    ...
    body.respostas.forEach((resposta, index) => {
      if (!resposta.id_sintoma) {
        erros.push(`respostas[${index}].id_sintoma é obrigatório.`);
      }
...
async function criarAvaliacao(req, res) {
  try {
    ...
    const resultado = await avaliacoesService.criarAvaliacao(
      req.body,
